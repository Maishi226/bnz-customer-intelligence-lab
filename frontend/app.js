...
function renderCampaign(c) {
  ...
  $("campaign").innerHTML = `<div class="campaign-grid">
    ...
    <article class="evidence card"><div class="card-head"><div><span class="kicker">CUSTOMER ASSURANCE</span><h3>Impact scorecard</h3></div><span style="max-width:190px;border-radius:99px;padding:6px 9px;background:#e9f5ff;color:#00549b;font-size:8px;font-weight:800;text-align:right">${escapeHtml(state.report.pipeline?.evaluation || state.report.ai_status || "Evaluation source unavailable")}</span></div><div class="scores">${scoreLabels.map(([key, label]) => `<div class="score"><b>${escapeHtml(c.scores[key])}</b><span>${label}</span></div>`).join("")}</div><div class="rewrite"><b>SAFER REWRITE</b><p>${escapeHtml(c.safer_rewrite)}</p></div><ul class="recommendations">${(c.recommendations || []).map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul></article>
